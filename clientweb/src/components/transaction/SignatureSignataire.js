...
const SignatureSignataire = () => {
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch(
          `http://51.178.39.67.nip.io/api/public/transactions/${id}/docs`
        );
  ...
  const pdfUrlBase = selectedDocId
    ? `http://51.178.39.67.nip.io/api/public/docs/${selectedDocId}/pdf`
    : firstDocId
    ? `http://51.178.39.67.nip.io/api/public/docs/${firstDocId}/pdf`
    : "";
