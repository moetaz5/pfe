...
const Dashboard = () => {
    ...
    const fetchOrg = async () => {
      try {
        const res = await axios.get(
          "http://51.178.39.67.nip.io/api/organizations/mine",
    ...
    const fetchDashboardData = async () => {
      try {
        if (isAdmin) {
          ...
          const res = await axios.get(
            "http://51.178.39.67.nip.io/api/dashboard/stats",
            { withCredentials: true },
          );
          const data = res.data;
          setStats({
  ...
  useEffect(() => {
    ...
    const fetchNotifs = async () => {
      try {
        const res = await axios.get("http://51.178.39.67.nip.io/api/notifications", {
          withCredentials: true,
        });
  ...
  const markAsRead = async (id) => {
    try {
      await axios.put(
        `http://51.178.39.67.nip.io/api/notifications/${id}/read`,
        {},
        { withCredentials: true },
      );
  ...
  const markAllAsRead = async () => {
    try {
      await axios.put(
        "http://51.178.39.67.nip.io/api/notifications/read-all",
        {},
        { withCredentials: true },
      );
  ...
  const deleteNotification = async (id) => {
    try {
      await axios.delete(`http://51.178.39.67.nip.io/api/notifications/${id}`, {
        withCredentials: true,
      });
