...
const Register = () => {
  ...
  const validateFullName = (fullName) => {
    ...
    // au moins deux mots
    const parts = trimmed.split(" ").filter(word => word.length > 0);

    if (parts.length < 2) {
      return false;
    }

    // chaque mot minimum 2 caractères
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateFullName(name)) {
      setError("Veuillez entrer votre nom complet (Nom + Prénom)");
      return;
    }
    ...
    try {
      const res = await axios.post("http://51.178.39.67.nip.io/api/auth/register", {
        name: name.trim(),
        email,
        password,
        phone,
      ...
      <div className="login-card">
        ...
        <form onSubmit={handleSubmit}>
          <div className="input-group">
            <input
              type="text"
              placeholder="Nom complet (Nom + Prénom)"
              value={name}
