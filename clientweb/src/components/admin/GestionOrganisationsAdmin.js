...
const GestionOrganisationsAdmin = () => {
  ...
  const fetchOrganizations = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://51.178.39.67.nip.io/api/admin/organizations/all", { withCredentials: true });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://51.178.39.67.nip.io/api/admin/organizations/${id}`, { withCredentials: true });
  ...
  const handleViewDetails = async (org) => {
    ...
    try {
      const [membersRes, txsRes] = await Promise.all([
        axios.get(`http://51.178.39.67.nip.io/api/admin/organizations/${org.id}/members`, { withCredentials: true }),
        axios.get(`http://51.178.39.67.nip.io/api/admin/organizations/${org.id}/transactions`, { withCredentials: true })
      ]);
