...
const GestionTransactionsAdmin = () => {
  ...
  const fetchTransactions = async () => {
    try {
      setLoading(true);
      const res = await axios.get("/api/admin/transactions/all", { withCredentials: true });
  ...
  const viewDetails = async (tx) => {
    ...
    setLoadingDocs(true);
    try {
      const res = await axios.get(`/api/admin/transactions/${tx.id}/documents`, { withCredentials: true });
      setTxDocs(res.data || []);
    } catch (err) {
  ...
  const handleDownloadZip = async (id) => {
    try {
      const res = await axios.get(`/api/admin/transactions/${id}/zip`, {
        withCredentials: true,
        responseType: "blob",
      });
  ...
  const handleResendTTN = async (id) => {
    try {
      await axios.post(`/api/admin/transactions/${id}/resend-ttn`, {}, { withCredentials: true });
