...
const GestionTransactionsAdmin = () => {
  ...
  const fetchTransactions = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://51.178.39.67.nip.io/api/admin/transactions/all", { withCredentials: true });
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    try {
      await axios.put(`http://51.178.39.67.nip.io/api/admin/transactions/${id}/status`, 
        { statut: newStatus }, 
  ...
  const viewDetails = async (tx) => {
    ...
    setLoadingDocs(true);
    try {
      const res = await axios.get(`http://51.178.39.67.nip.io/api/admin/transactions/${tx.id}/documents`, { withCredentials: true });
      setTxDocs(res.data || []);
    } catch (err) {
  ...
  const handleDownloadZip = async (id) => {
    try {
      const res = await axios.get(`http://51.178.39.67.nip.io/api/admin/transactions/${id}/zip`, {
        withCredentials: true,
        responseType: "blob",
      });
  ...
  const handleResendTTN = async (id) => {
    try {
      await axios.post(`http://51.178.39.67.nip.io/api/admin/transactions/${id}/resend-ttn`, {}, { withCredentials: true });
