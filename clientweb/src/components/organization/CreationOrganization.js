...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const orgRes = await axios.post(
      "http://51.178.39.67.nip.io/api/organizations",
      {
        name: form.name,
    ...
    // 2️⃣ Envoyer invitations email une par une
    for (const email of form.invitedUsers) {
      try {
        await axios.post(
          `http://51.178.39.67.nip.io/api/organizations/${organizationId}/invite`,
          { email },
          { withCredentials: true }
        );
        ...
        if (inviteErr.response?.data?.message) {
          toast.warning(
            `⚠ ${email} : ${inviteErr.response.data.message}`
          );
        } else {
          toast.warning(`⚠ Erreur invitation ${email}`);
        }
      }
    }
