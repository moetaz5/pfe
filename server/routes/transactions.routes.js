module.exports = function(app, locals) {
  const {
    ...
    generateQrPngBase64, stampPdfWithTTN, generateXmlFromPdf, bufferToB64, b64ToBuffer, ensureBase64String,
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const autoGenerateXml = req.body.auto_generate_xml === "true";

        if (!autoGenerateXml && pdfFiles.length !== xmlFiles.length) {
          // Rollback
          await db.promise().query("DELETE FROM transactions WHERE id = ?", [transactionId]);
        ...
        for (let i = 0; i < pdfFiles.length; i++) {
          let xmlB64;
          if (autoGenerateXml) {
            console.log(`[AI Gen XML] Génération du XML pour le PDF : ${pdfFiles[i].originalname}`);
            try {
              const xmlText = await generateXmlFromPdf(pdfFiles[i].buffer);
              console.log(`[AI Gen XML] XML généré avec succès pour : ${pdfFiles[i].originalname}`);
              xmlB64 = Buffer.from(xmlText).toString("base64");
            } catch (aiErr) {
              console.error(`[AI Gen XML Error] pour ${pdfFiles[i].originalname} :`, aiErr.message);
              // Rollback la transaction créée pour garder la base propre
              await db.promise().query("DELETE FROM transactions WHERE id = ?", [transactionId]);
              return res.status(500).json({
                message: `Échec de l'extraction IA du XML à partir du PDF : ${aiErr.message}. Veuillez fournir un fichier XML manuellement.`
              });
            }
          } else {
            xmlB64 = xmlFiles[i].buffer.toString("base64");
          }
  ...
  // 🚀 RENVOYER À LA TTN (MANUELLEMENT SI RÉFUSÉ OU OUBLIÉ) - VERSION DÉTAILLÉE
  app.post("/api/transactions/:id/resend-ttn-detail", verifyToken, async (req, res) => {
    const { id } = req.params;
