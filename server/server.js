...
//**TTN*================ */
const TTN_URL =
  "https://leamonpro.pythonanywhere.com/ElfatouraServices/EfactService";
...
const stampPdfWithTTN = async ({
  ...
  if (qrPngB64) {
    ...
    let y = height - Number(qrConf.qrPositionY || 0) - qrH;
  ...
  if (ttnReference) {
    ...
    let y = height - Number(refConf.labelPositionY || 0);
...
app.post("/api/auth/certify", verifyToken, async (req, res) => {
  try {
    ...
    if (!matricule_fiscale || !adresse || !ville || !code_postal || !ttn_login) {
      return res.status(400).json({
...
const [userCheck] = await db.promise().query(
  "SELECT certified FROM users WHERE id = ?",
  [req.user.id]
);

if (!userCheck.length || userCheck[0].certified === 0) {
  return res.status(403).json({
    message: "Votre compte doit être certifié avant de créer une transaction",
  });
}
      // 🔥 CONFIGURATION PROPRE
      const qrConfig =
        typeof qr_config === "string"
          ? JSON.parse(qr_config)
          : qr_config || {};
...
app.get(
  ...
  async (req, res) => {
    ...
    docs.forEach((d) => {
      ...
      } else if (d.statut === "signée" && d.xml_signed) {
...
app.get("/api/transactions/:id/download", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (fileType === "xml") {
      ...
      else if (doc.statut === "signée" && doc.xml_signed)
        fileBuffer = doc.xml_signed;
      else
        fileBuffer = doc.xml_file;
...
app.get("/api/statistiqueadmin", verifyToken, async (req, res) => {
  try {
    ...
    const [usersCountResult, txRowsResult, docsRowsResult, txByMonthResult] =
      await Promise.all([
        // Total users
...
app.post("/api/public/transactions/:id/sign", async (req, res) => {
  ...
  try {
    ...
    // ================= SIGNATURE =================
    const qrConfig = resolveConfig(transaction.qr_config);
    const refConfig = resolveConfig(transaction.ref_config);
    ...
    for (const doc of docs) {
      ...
      // 📡 SAVE TTN
      const saveRes = await saveEfactTTN(signedXmlB64);
  ...
  `La transaction #${id} a été signée avec succès`,
  "success"
);

    // 📥 Récupérer PDFs signés
    const [signedDocs] = await db.promise().query(
      `
      SELECT filename, pdf_file
      FROM transaction_documents
      WHERE transaction_id = ? AND statut = 'signée_ttn'
      `,
      [id]
    );

    // 📧 ENVOI EMAIL CLIENT
    if (transaction.client_email && signedDocs.length) {
    ...
    res.json({
      success: true,
      message: "Transaction signée (-1 jeton) + email client envoyé"
    });

  } catch (err) {
    console.error("SIGN TTN ERROR:", err);
...
app.post("/api/organizations/:id/invite", verifyToken, async (req, res) => {
  try {
    ...
    const [existingMember] = await db.promise().query(
      `SELECT id FROM organization_members WHERE user_id = ?`,
      [invitedUserId]
    );

    if (existingMember.length)
      return res.status(409).json({ message: "Déjà membre d'une organisation" });
...
app.get("/api/organizations/invitations/mine", verifyToken, async (req, res) => {
  try {
    const [rows] = await db.promise().query(
      `
      SELECT oi.*, o.name AS organization_name
...
app.post("/api/support/contact", verifyToken, async (req, res) => {
  try {
    const SUPPORT_EMAIL = "amri.aymen@medicacom.tn";
    ...
    const [rows] = await db
      .promise()
      .query("SELECT id, name, email, phone FROM users WHERE id = ?", [req.user.id]);
