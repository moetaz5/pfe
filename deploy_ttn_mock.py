...
def deploy_ttn_mock():
    ...
    try:
        ...
        print("\n--- Installation des dependances Python (flask, flasgger, qrcode) ---")
        stdin, stdout, stderr = ssh.exec_command('sudo apt-get update && sudo apt-get install -y python3-pip && pip3 install flask flasgger qrcode pillow')
        print(stdout.read().decode())
        print(stderr.read().decode())
        
        # 2. Start or restart the mock server with PM2
        print("\n--- Demarrage de TTN Mock Server avec PM2 ---")
        cmd = 'cd /var/www/medica_sign && pm2 start ttn_mock_server.py --name ttn_mock --interpreter python3 || pm2 restart ttn_mock'
